...
# define DIM 2 /* a linear problem with 6 varibles */
# define CONS 0 /* a linear problem with 5 linear constraints */
...
void objfun(int n, int m, double *x, double *fx)
{
  ...
  for(j=0;j<m;j++){
	fx[j]=100*pow((x[j*n+1]-pow(x[j*n],2.0)),2.0)+pow(1-x[j*n],2.0);
  }
